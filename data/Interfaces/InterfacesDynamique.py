...
class Carte(QMainWindow):
    ...
    def __init__(self):
        ...
        # Liste des extensions d'images à inclure
        image_extensions = ["jpeg", "jpg", "png", "bmp"]

        # Liste pour stocker les chemins d'accès aux images d'arrière-plan
        self.background_images = []
        # Parcours des fichiers dans le dossier avec les extensions spécifiées
        for extension in image_extensions:
            # Utilisation de glob.glob() pour rechercher les fichiers avec l'extension actuelle
            files = glob.glob(os.path.join(background_folder, f"*.{extension}"))
            # Ajoute tous les fichiers trouvés à la liste des images d'arrière-plan
            self.background_images.extend(files)
    ...
    def change_background(self):
        ...
        """
        # Incrémente l'index de l'image d'arrière-plan de 1,
        # puis prend le reste de la division par la longueur de la liste
        # Cela garantit que l'index reste dans la plage des indices valides de la liste des images d'arrière-plan.
